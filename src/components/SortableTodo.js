...
import { Card, CardHeader, CardText, Button, Col, CardBody } from 'reactstrap';
...
const SortableTodo = SortableElement(({ value, index, hiddeTodo, removeTodo }) => {
  ...
  return (
    <Col sm="4" className="mt-3 todo-card">
      <Card className="border border-info shadow-lg" style={{ opacity: value.disable ? '0.2' : '1' }}>
        ...
        <CardBody>
          ...
          <Button outline color="secondary" className="float-right" onClick={disableTodo(index)}>                {value.disable ? 'now' : 'after'}
          </Button>
        </CardBody>
      </Card>
    </Col>
